...
## scores
stt <- function(x, location = 0, scale = 1, df, left = -Inf, right = Inf, which = NULL, drop = TRUE, eps = .Machine$double.eps^(1/3)) {
  stopifnot(
  ...
  scr <- function(par) switch(par,
    "df" = (dtt(x, location = location, scale = scale, df = df + eps, left = left, right = right, log = TRUE) - 
            dtt(x, location = location, scale = scale, df = df - eps, left = left, right = right, log = TRUE))/(2 * eps),
    "mu"  = .Call("stt_mu", x, location, scale, df, left, right),
    "sigma" = .Call("stt_sigma", x, location, scale, df, left, right))
...
## Hessian
htt <- function(x, location = 0, scale = 1, df, left = -Inf, right = Inf, which = NULL, drop = TRUE, expected = FALSE, eps = .Machine$double.eps^(1/4)) {
  ...
  scr <- function(par, eps) {
    ...
    stt(x, location = location, scale = scale, df = df, left = left, right = right, which = par[1L])
  }
  hess <- function(par) switch(par,
    "mu"    = .Call("htt_mu", x, location, scale, df, left, right),
    "sigma" = .Call("htt_sigma", x, location, scale, df, left, right),
    "mu:sigma" = .Call("htt_musigma", x, location, scale, df, left, right),
